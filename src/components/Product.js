    ...
    const handleClick = (obj) => {
        ...
        db.collection('users').doc(currentUser.id).collection('cart').doc(props.product.id).get()
            .then(productRef => {
                ...
                // otherise if the product.id has not been found in the currentUser's cart
                else {
                    // The quantity attribute does not yet exists, so we will create it and set its value to 1
                    data = obj;
                    data.quantity = 1;
                }
